...
const actionSchema = z.object({
  kind: z.enum(["none", "milestone", "risk", "investment_scenario", "crisis_plan"]),
  operation: z.enum(["none", "create", "update"]),
  recordId: z.number().int().nullable(),
  title: z.string().nullable(),
  targetDate: z.string().nullable(),
  status: z.enum(["planned", "in_progress", "done", "blocked"]).nullable(),
  severity: z.enum(["low", "medium", "high", "critical"]).nullable(),
  likelihood: z.enum(["low", "medium", "high"]).nullable(),
  mitigationNotes: z.string().nullable(),
  fundingAmount: z.string().nullable(),
  valuation: z.string().nullable(),
  runwayMonths: z.number().int().nullable(),
  useOfFunds: z.string().nullable(),
  triggerConditions: z.string().nullable(),
  responseSteps: z.string().nullable(),
  owner: z.string().nullable(),
  riskId: z.number().int().nullable(),
}).strict();
    ...
    try {
      ...
      const modelResult = await invokeLLM({
        ...
        response_format: {
          type: "json_schema",
          json_schema: {
            name: "venture_advisor_reply",
            strict: true,
            schema: z.toJSONSchema(chatResponseSchema),
          },
        },
      });

      const content = modelResult.choices[0]?.message?.content;
      if (typeof content !== "string") throw new Error("The advisor returned no reply.");
      const advisor = chatResponseSchema.parse(JSON.parse(content));
