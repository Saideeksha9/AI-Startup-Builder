...
export default function LandingPreview() {
  ...
  if (!blueprint) {
    return <main className="min-h-screen bg-slate-50"><AppNavigation /><div className="flex min-h-[calc(100vh-72px)] items-center justify-center p-6"><section className="max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm"><Rocket className="mx-auto h-8 w-8 text-blue-600" aria-hidden="true" /><h1 className="mt-4 text-2xl font-black text-black">No landing page preview is available.</h1><p className="mt-3 text-sm leading-6 text-slate-600">Return to the startup builder, generate a blueprint, then use its landing-page CTA to open this interactive preview.</p><Button type="button" onClick={() => window.close()} className="mt-6 rounded-full bg-slate-950 text-white">Close preview</Button></section></div></main>;
  }
